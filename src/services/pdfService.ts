...
function _buildPDF(
    ...
    if (memory.moatRationale) {
        doc.fontSize(8.5).fillColor(GRAY).font('Helvetica-Oblique')
            .text(sanitizeText(memory.moatRationale), 40, y, { width: doc.page.width - 80 });
    }
    ...
    for (const scenarioId of ['RECESSION', 'PRICE_WAR', 'SCALE_UP', 'TALENT']) {
        ...
        doc.fontSize(9).fillColor(RED).font('Helvetica-Bold')
            .text(`Overall Score: ${overallScore} -> ${stressedAvg}  (Delta: ${stressedAvg - overallScore} pts)`, 44, y + 6);
        y += 30;
